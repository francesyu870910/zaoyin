        ...
        <div id="confirmModal" class="modal" style="display: block;">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="text-align: right; margin-top: 20px;">
                    ...
                    ${onConfirm ? `<button type="button" class="btn btn-primary" onclick="confirmAction()" style="margin-left: 10px;">${confirmText}</button>` : ''}
                </div>
            </div>
        </div>
    `;
...
async function showAlertDetails(alertId) {
    try {
        ...
        if (response.success) {
            ...
            const details = `
                监测点: ${alert.point_name}
                地  点: ${alert.location}
                告警时间: ${new Date(alert.alert_time).toLocaleString()}
                噪声等级: ${alert.noise_level} dB
                告警阈值: ${alert.threshold_value} dB
                处理状态: ${alert.status === 'pending' ? '待处理' : '已处理'}
                处 理 人: ${alert.handler_name || 'N/A'}
                处理时间: ${alert.handled_at ? new Date(alert.handled_at).toLocaleString() : 'N/A'}
            `;
            showConfirmDialog(
                '告警详情',
                `告警 ID: ${alert.id}`,
        ...
        showAlert('获取告警详情失败: ' + error.message, 'danger');
    }
}
