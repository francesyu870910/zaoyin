    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const users = [
                ...
                { username: 'operator1', password: 'op123456', role: 'user' },
                { username: 'operator2', password: 'op123456', role: 'user' },
                ...
                db.get('SELECT id FROM users WHERE username = ?', [userData.username], (err, user) => {
                    ...
                    if (!user) {
                        db.run('INSERT INTO users (username, password, role) VALUES (?, ?, ?)', 
                            [userData.username, hashedPassword, userData.role], (err) => {
                                if (err) {
...
function generateAlertData(db, callback) {
    ...
    db.all(sql, (err, exceedData) => {
        ...
        const insertAlert = db.prepare(`
            INSERT INTO alerts (point_id, noise_level, threshold_value, alert_time, status) 
            VALUES (?, ?, ?, ?, ?)
        `);
